...
# # 개별 ggplot2 그래프 생성
# p1 <- ggplot(students_foreigner %>% filter(학과 == '경제학부'), aes(x = 국적, y = 학생수)) +   
#   geom_bar(stat = "identity") +
#   ggtitle("경제학부 외국인 학생")
# 
# p2 <- ggplot(students_foreigner %>% filter(학과 == '경영학부'), aes(x = 국적, y = 학생수)) +
#   geom_bar(stat = "identity") +
#   ggtitle("경영학부 외국인 학생")
# 
# # 두 개의 그래프를 2x2 레이아웃에 배치 (빈 공간 포함)
...
p11 <- ggplot(students_dept %>% filter(학과 == '경제학부'), aes(x = 국적, y = 학생수)) +   
...
p12 <- ggplot(students_dept %>% filter(학과 == '경영학부'), aes(x = 국적, y = 학생수)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = 학생수), vjust = -0.5, size = 3) +  # Add student count on top of bars
  scale_x_discrete(limits = all_nationalities) +  # 동일한 x축 범주 사용
  ylim(0, max_y) +
...
p13 <- ggplot(students_dept %>% filter(학과 == '보험계리학과'), aes(x = 국적, y = 학생수)) +   
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = 학생수), vjust = -0.5, size = 3) +  # Add student count on top of bars
  scale_x_discrete(limits = all_nationalities) +  # 동일한 x축 범주 사용
  ylim(0, max_y) +
...
p14 <- ggplot(students_dept %>% filter(학과 == '회계세무학과'), aes(x = 국적, y = 학생수)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = 학생수), vjust = -0.5, size = 3) +  # Add student count on top of bars
  scale_x_discrete(limits = all_nationalities) +  # 동일한 x축 범주 사용
  ylim(0, max_y) +
